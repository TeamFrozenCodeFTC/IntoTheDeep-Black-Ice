...
//    public HeadingCorrections headingCorrection;
//
//    public Movement(Robot robot) {
//        this.robot = robot;
//        this.target = new TargetTracker(robot);
//        this.headingCorrection = new HeadingCorrections(this);
//    }
//
...
//
//        double X = target.xError * 0.5;
//        double Y = target.yError * 0.5;
//
//        double[] powers = fieldVectorToLocalWheelPowers(X, Y, 1);
//
//        robot.drive.power(headingCorrection.applyHeadingCorrection(powers, headingCorrection.turnOverMovement()));
//    }
//
...
//
//        boolean didBrake = false;
//
//        while (robot.isNotInterrupted()) {
//            robot.telemetry.addData("moving", "to position");
...
//                break;
//            }
//            else if (!didBrake && target.isWithinBrakingDistance()) {
//                if (robot.odometry.velocity < 5) {
//                    didBrake = true;
//                    continue;
//                }
...
//
//        while (robot.isNotInterrupted() && target.isNotWithinErrorMargin(target.defaultErrorMargin)) {
//            if (target.isWithinBrakingDistance()) {
//                break;
//            }
//            moveTowardTarget(1, headingCorrection.turnOverMovement());
//
...
//
//        while (robot.isNotInterrupted() && target.isNotWithinErrorMargin(target.defaultErrorMargin)) {
//            moveTowardTarget2(1, headingCorrection.turnOverMovement());
//
//            robot.loopUpdate();
//        }
//    }
...
//        do {
//            robot.loopUpdate();
//            robot.drive.power(headingCorrection.applyHeadingCorrection(robot.drive.backward(power), headingCorrection.turnOverMovement()));
...
////        if (Math.abs(x1)/linearSlow < 1) {
////            x1 = target.xError;
////        }
////        if (Math.abs(y1)/linearSlow < 1) {
////            y1 = target.yError;
////        }
//
//        double[] powers = fieldVectorToLocalWheelPowers(x1, y1, linearSlow);
//
//        robot.drive.power(headingCorrection.applyHeadingCorrection(powers, turnMethod));
//    }
//
//    public void nonPIDStopAt(double heading, double x, double y) {
//        target.setTarget(heading, x, y);
//
//        while (robot.isNotInterrupted() && target.isNotWithinErrorMargin(target.defaultErrorMargin)) {
//            nonPIDStop();
...
//    }
//
//    public void nonPIDStop() {
//        double x1 = target.xError - robot.odometry.xBrakingDistance;
//        double y1 = target.yError - robot.odometry.yBrakingDistance;
//
//        double[] powers = fieldVectorToLocalWheelPowers(x1, y1, 1);
//
//        robot.drive.power(headingCorrection.applyHeadingCorrection(powers, headingCorrection.turnOverMovement()));
//    }
//
//    public void moveTowardTarget2(double linearSlow, double turnMethod) {
//        double x1 = target.xError * 0.8;
//        double y1 = target.yError * 0.8;
//
//        double[] powers = fieldVectorToLocalWheelPowers(x1, y1, linearSlow);
//
//        robot.drive.power(headingCorrection.applyHeadingCorrection(powers, turnMethod));
